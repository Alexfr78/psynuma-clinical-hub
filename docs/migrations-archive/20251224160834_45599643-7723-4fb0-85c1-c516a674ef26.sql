-- Add conversion tracking fields to calendar_events table
ALTER TABLE calendar_events ADD COLUMN IF NOT EXISTS is_converted boolean DEFAULT false;
ALTER TABLE calendar_events ADD COLUMN IF NOT EXISTS converted_session_id uuid NULL REFERENCES sessions(id) ON DELETE SET NULL;
ALTER TABLE calendar_events ADD COLUMN IF NOT EXISTS converted_at timestamp with time zone;

-- Create index for efficient filtering
CREATE INDEX IF NOT EXISTS idx_calendar_events_is_converted ON calendar_events(is_converted) WHERE is_converted = false;

-- Create RPC function to convert calendar event to session transactionally
...
BEGIN
  ...
  SELECT center_id INTO v_center_id FROM profiles WHERE id = v_event.professional_id;
