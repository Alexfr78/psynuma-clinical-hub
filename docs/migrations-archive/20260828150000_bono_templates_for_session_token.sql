-- Allows the public appointment page (/cita/:token) to offer bono purchases
-- alongside single-session payment, reusing the same token-scoped pattern
-- used by the debt payment portal (get_public_bono_templates_for_debt).
