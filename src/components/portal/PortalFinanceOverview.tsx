  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-3" aria-label="Resumen de pagos y bonos">
        <Card><CardContent className="flex min-h-28 items-center gap-3 p-4"><div className="rounded-lg bg-destructive/10 p-3 text-destructive"><Icon name="error" className="h-5 w-5" aria-hidden="true" /></div><div><p className="text-sm text-muted-foreground">Pendiente</p><p className="text-xl font-semibold tabular-nums">{money.format(pendingTotal)}</p></div></CardContent></Card>
        <Card><CardContent className="flex min-h-28 items-center gap-3 p-4"><div className="rounded-lg bg-primary/10 p-3 text-primary"><Icon name="inventory" className="h-5 w-5" aria-hidden="true" /></div><div><p className="text-sm text-muted-foreground">Sesiones en bonos</p><p className="text-xl font-semibold tabular-nums">{remainingSessions}</p></div></CardContent></Card>
        <Card><CardContent className="flex min-h-28 items-center gap-3 p-4"><div className="rounded-lg bg-muted p-3 text-foreground"><Icon name="receipt_long" className="h-5 w-5" aria-hidden="true" /></div><div><p className="text-sm text-muted-foreground">Pagos registrados</p><p className="text-xl font-semibold tabular-nums">{data.payments.length}</p></div></CardContent></Card>
      </div>

      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Icon name="payments" className="h-5 w-5 text-primary" aria-hidden="true" />Importes pendientes</CardTitle><CardDescription>Pagos que todavía están pendientes</CardDescription></CardHeader>
        <CardContent>
          {pendingDebts.length === 0 ? (
            <div className="flex items-center gap-3 rounded-lg border border-dashed p-4 text-sm text-muted-foreground"><Icon name="check_circle" className="h-5 w-5 text-primary" aria-hidden="true" />No tienes pagos pendientes.</div>
          ) : (
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Icon name="inventory" className="h-5 w-5 text-primary" aria-hidden="true" />Mis bonos</CardTitle><CardDescription>Sesiones disponibles y fecha de validez</CardDescription></CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Icon name="receipt_long" className="h-5 w-5 text-primary" aria-hidden="true" />Historial de pagos</CardTitle><CardDescription>Últimos pagos registrados por el centro</CardDescription></CardHeader>
        <CardContent>
          {data.payments.length === 0 ? <p className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">Todavía no hay pagos registrados.</p> : (
            <div className="divide-y rounded-xl border">
              {data.payments.map((payment) => {
                ...
                return <div key={payment.id} className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{payment.concept}</p>{refunded && <Badge variant="outline"><Icon name="restart_alt" className="mr-1 h-3 w-3" aria-hidden="true" />Reembolso</Badge>}</div><p className="mt-1 text-sm text-muted-foreground">{displayDate(payment.paymentDate)} - {paymentMethodLabel(payment.paymentMethod)}</p></div><div className="text-left sm:text-right"><p className="font-semibold tabular-nums">{money.format(payment.amount)}</p>{refunded && <p className="text-xs text-muted-foreground">Reembolsado: {money.format(payment.refundedAmount)}</p>}</div></div>;
              })}
            </div>
          )}
        </CardContent>
