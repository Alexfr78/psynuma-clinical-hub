...
export function PortalDocuments({ documents, loading }: PortalDocumentsProps) {
  if (loading) return <div className="flex min-h-40 items-center justify-center" role="status"><Icon name="progress_activity" className="mr-2 h-5 w-5 animate-spin text-muted-foreground" aria-hidden="true" /><span className="text-sm text-muted-foreground">Cargando documentos...</span></div>;
  if (documents.length === 0) return <div className="flex flex-col items-center rounded-xl border border-dashed px-4 py-12 text-center"><Icon name="task" className="h-10 w-10 text-muted-foreground" aria-hidden="true" /><h2 className="mt-4 text-lg font-semibold">No tienes documentos</h2><p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">Cuando tu profesional te envíe un consentimiento, evaluación o autorregistro, aparecerá aquí.</p></div>;
