CREATE OR REPLACE FUNCTION public.recompute_debt_by_invoice_service(p_debt_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_invoice_id IS NULL THEN
    RETURN jsonb_build_object('debt_id', p_debt_id, 'skipped', true, 'reason', 'no_invoice');
  END IF;
...
REVOKE ALL ON FUNCTION public.recompute_debt_by_invoice_service(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.recompute_debt_by_invoice_service(uuid) FROM anon;
REVOKE ALL ON FUNCTION public.recompute_debt_by_invoice_service(uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.recompute_debt_by_invoice_service(uuid) TO service_role;