...
-- Enums, tablas, indices, triggers, seed de categorias, RLS, bucket de
-- Storage, RPC de calculo de comision variable y crons de automatizacion.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- 1. Categorias de gasto (configurables por centro, con set por defecto)
-- ---------------------------------------------------------------------
CREATE TABLE public.expense_categories (
  ...
  icon text, -- nombre de glifo Material Symbols, opcional
  is_professional_payment_category boolean NOT NULL DEFAULT false, -- marca la categoria reservada para pagos a profesionales
...
-- Semilla por defecto para TODOS los centros existentes hoy.
...
-- Trigger para sembrar automaticamente las categorias por defecto en cada
-- centro nuevo.
...
-- Protege la categoria reservada de pagos a profesionales: si se borrara o
-- desactivara, el cron generate-professional-payments dejaria de poder
-- liquidar. La UI ya lo impide; esto lo garantiza tambien a nivel de BD.
...
-- ---------------------------------------------------------------------
-- 2. Proveedores (facturas de proveedores con datos fiscales)
-- ---------------------------------------------------------------------
CREATE TABLE public.suppliers (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  center_id uuid NOT NULL REFERENCES public.centers(id) ON DELETE CASCADE,
  name text NOT NULL,
  tax_id text, -- NIF/CIF del proveedor, validado en el cliente con src/lib/nif-validation.ts
  address text,
  city text,
  postal_code text,
  province text,
...
-- ---------------------------------------------------------------------
-- 3. Plantillas de gasto recurrente (la "regla"; genera anotaciones)
-- ---------------------------------------------------------------------
CREATE TABLE public.expense_recurring_templates (
  ...
  -- Dia del mes/trimestre/anio en que se genera la anotacion (1-28 para evitar
  -- problemas con meses de 28-31 dias; frontend valida el rango).
  day_of_period smallint NOT NULL DEFAULT 1 CHECK (day_of_period BETWEEN 1 AND 28),
  -- Mes de inicio del ciclo para 'quarterly'/'yearly' (1-12); irrelevante para 'monthly'.
  anchor_month smallint CHECK (anchor_month BETWEEN 1 AND 12),
  is_active boolean NOT NULL DEFAULT true,
  starts_on date NOT NULL DEFAULT CURRENT_DATE,
  ends_on date, -- opcional, null = indefinido
  default_payment_method text,
  vat_rate numeric(5,2), -- IVA soportado por defecto, si aplica
  irpf_rate numeric(5,2), -- retencion IRPF por defecto, si aplica
  last_generated_period date, -- primer dia del ultimo periodo ya generado, evita duplicados
...
-- ---------------------------------------------------------------------
-- 4. Acuerdos de compensacion por profesional (fijo / % / mixto)
-- ---------------------------------------------------------------------
CREATE TABLE public.professional_compensation_agreements (
  ...
  fixed_amount numeric(10,2) NOT NULL DEFAULT 0 CHECK (fixed_amount >= 0), -- usado si type IN ('fixed','mixed')
  percentage_rate numeric(5,2) NOT NULL DEFAULT 0 CHECK (percentage_rate BETWEEN 0 AND 100), -- usado si type IN ('percentage','mixed')
  compensation_basis public.compensation_basis NOT NULL DEFAULT 'collected_payments',
  -- Retencion IRPF por defecto a aplicar a las liquidaciones generadas por el
  -- cron mensual para este acuerdo. Null = no calcular retencion automaticamente
  -- (el admin decide caso a caso si el profesional factura como autonomo).
  default_irpf_rate numeric(5,2),
  category_id uuid REFERENCES public.expense_categories(id) ON DELETE SET NULL, -- categoria a usar al generar el gasto (por defecto "Pagos a profesionales")
  is_active boolean NOT NULL DEFAULT true,
  effective_from date NOT NULL DEFAULT CURRENT_DATE,
  effective_to date, -- null = vigente indefinidamente; al crear un acuerdo nuevo se cierra el anterior con effective_to
  notes text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
...
-- Solo un acuerdo activo (effective_to IS NULL) por profesional a la vez.
...
-- ---------------------------------------------------------------------
-- 5. Gastos (anotaciones) -- tabla principal
-- ---------------------------------------------------------------------
CREATE TABLE public.expenses (
  ...
  -- Solo relevante si kind = 'professional_payment'
  professional_id uuid REFERENCES public.profiles(id) ON DELETE SET NULL,
  compensation_agreement_id uuid REFERENCES public.professional_compensation_agreements(id) ON DELETE SET NULL,
  compensation_period_start date, -- primer dia del periodo liquidado
  compensation_period_end date,   -- ultimo dia del periodo liquidado

  -- Solo relevante si kind = 'fixed_recurring' (anotacion generada por una plantilla)
  recurring_template_id uuid REFERENCES public.expense_recurring_templates(id) ON DELETE SET NULL,
  -- Primer dia del periodo generado por la plantilla recurrente (columna
  -- genérica, separada de compensation_period_start que es solo para
  -- professional_payment -- ver nota de claridad del diseño).
  generated_period_start date,

  description text NOT NULL,
  amount numeric(10,2) NOT NULL CHECK (amount >= 0), -- importe editable incluso si viene de una plantilla

  -- Datos fiscales de factura de proveedor
  tax_base numeric(10,2), -- base imponible
  vat_rate numeric(5,2),  -- % IVA soportado
  vat_amount numeric(10,2), -- cuota IVA
  irpf_rate numeric(5,2), -- % retencion IRPF
  irpf_amount numeric(10,2), -- cuota retenida
  supplier_invoice_number text, -- numero de factura del proveedor
  invoice_issue_date date, -- fecha de expedicion de la factura recibida
  operation_date date, -- fecha de la operacion, si difiere de la de expedicion

  expense_date date NOT NULL DEFAULT CURRENT_DATE, -- fecha del gasto/devengo
  ...
  -- Justificante (Storage) + extraccion IA
  attachment_path text, -- ruta en el bucket 'expense-receipts': {center_id}/{expense_id}/{uuid}.{ext}
  attachment_mime_type text,
  ai_extraction_status text CHECK (ai_extraction_status IN ('pending','processing','done','failed')),
  ai_extraction_raw jsonb, -- respuesta cruda de la IA para depuracion/auditoria
  ai_extraction_confidence numeric(3,2), -- 0-1, opcional
...
-- Evita duplicar la anotacion de un mismo periodo para la misma plantilla
-- recurrente (idempotencia del cron, ver generate-recurring-expenses).
...
-- Evita duplicar la liquidacion de un mismo periodo para el mismo acuerdo de
-- compensacion (idempotencia del cron mensual, ver generate-professional-payments).
...
-- ---- expense_categories: visibles para todo el centro, gestionables solo por admin ----
...
-- ---- suppliers: visibles y gestionables por admin y professional del centro ----
...
-- ---- expense_recurring_templates: solo admin gestiona/ve plantillas recurrentes ----
...
-- ---- professional_compensation_agreements: solo admin gestiona/ve TODOS;
--      el propio profesional puede ver (no editar) su acuerdo ----
...
-- ---- expenses: el corazon del control de acceso ----
-- Admin: ve y gestiona TODOS los gastos del centro.
CREATE POLICY "Admins manage all expenses in center"
ON public.expenses FOR ALL TO authenticated
USING (center_id = public.get_user_center_id(auth.uid()) AND public.is_admin(auth.uid()))
WITH CHECK (center_id = public.get_user_center_id(auth.uid()) AND public.is_admin(auth.uid()));

-- Profesional: puede INSERTAR gastos propios (kind IN ('variable','fixed_recurring',
-- 'supplier_invoice') registrados por el mismo; 'professional_payment' NO lo puede
-- crear el mismo, solo el admin/el cron de liquidacion).
...
-- Profesional: ve solo los gastos que el mismo ha creado, o los "pagos a profesional"
-- que le correspondan a el como beneficiario (professional_id = auth.uid()).
...
-- Profesional: puede editar/eliminar SOLO los gastos que el mismo creo y que
-- siguen 'pending' (no puede tocar un pago a profesional ni un gasto ya conciliado).
-- USING exige status = 'pending' (solo se pueden tocar gastos aun no
-- conciliados); WITH CHECK no lo exige para permitir la transicion
-- pending -> paid al marcar como pagado un gasto propio.
...
-- Nota: las funciones que generan gastos recurrentes o de liquidacion de
-- profesionales (edge functions con cron) usan el cliente service_role, que
-- bypassa RLS por diseño de Supabase -- no necesitan politicas propias.

-- ---------------------------------------------------------------------
-- 7. Storage: bucket de justificantes
-- ---------------------------------------------------------------------
INSERT INTO storage.buckets (id, name, public)
VALUES ('expense-receipts', 'expense-receipts', false)
ON CONFLICT (id) DO NOTHING;

-- Ruta esperada: {center_id}/{expense_id}/{uuid}.{ext}
...
-- ---------------------------------------------------------------------
-- 8. Calculo de comision variable -- funcion interna compartida + RPC publica
-- ---------------------------------------------------------------------
-- Funcion interna SIN chequeo de auth.uid(): la usan tanto el RPC publico
-- (para la UI, que si exige auth.uid()) como el cron de liquidacion mensual
-- (service_role, sin sesion de usuario) -- evita que ambos caminos diverjan.
...
-- Esta función interna NO comprueba auth.uid() (la llama el cron con
-- service_role), así que debe quedar inalcanzable para 'authenticated'/'anon'
-- -- de lo contrario cualquier usuario autenticado podría consultar los
-- ingresos de cualquier profesional de cualquier centro pasando center_id
-- arbitrario. Postgres concede EXECUTE a PUBLIC por defecto: hay que revocarlo
-- explícitamente y conceder solo a service_role.
...
-- RPC publica: exige auth.uid() (admin del centro, o el propio profesional
-- consultando su propio importe). Usada por la UI de previsualizacion.
...
-- ---------------------------------------------------------------------
-- 9. Automatizacion: cron diario (gastos recurrentes) y cron mensual
--    (liquidacion de profesionales). Mismo mecanismo pg_cron + pg_net que
--    supabase/migrations/20260811122000_schedule_payment_automation.sql,
--    reutilizando el secreto de Vault 'payment_automation_cron_secret'
--    (debe coincidir con el secreto de Edge Function CRON_SECRET).
-- ---------------------------------------------------------------------
DO $$
BEGIN
